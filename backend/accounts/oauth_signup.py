...
SIGNUP_INTENT_SESSION_KEY = "_oauth_signup_intent"
SIGNUP_COMPLETE_URL_NAME = "oauth_signup_complete"
_SIGNUP_COMPLETE_URL_PATH = "/accounts/oauth-signup-complete/"
...
def write_intent(
    ...
    *,
    ...
    request.session[SIGNUP_INTENT_SESSION_KEY] = {
        "provider": provider,
...
def read_intent(
    ...
    intent = request.session.get(SIGNUP_INTENT_SESSION_KEY)
    if not intent:
        return None
...
def clear_intent(
    ...
    if SIGNUP_INTENT_SESSION_KEY in request.session:
        del request.session[SIGNUP_INTENT_SESSION_KEY]
        request.session.modified = True
...
def is_intent_expired(
    ...
    try:
        ...
        return True
    ttl = timedelta(seconds=settings.OAUTH_SIGNUP_INTENT_TTL_SECONDS)
...
def _complete_redirect(
    ...
    return ImmediateHttpResponse(
        HttpResponseRedirect(f"{_SIGNUP_COMPLETE_URL_PATH}{qs}"),
    )
