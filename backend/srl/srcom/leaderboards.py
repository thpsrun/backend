        ...
        for category in game.categories or []:
            ...
            for level in level_iter:
                ...
                for combo in combos:
                    ...
                    if lb_data:
                        dispatch_with_recon(sync_leaderboards, lb_data)
...
def sync_leaderboards(
    ...
    with check_reconciliation(recon_job_id):
        ...
        for lb_run in src_lb.runs:
            ...
            dispatch_with_recon(sync_run, run_context.model_dump())
...
def sync_run(
    ...
    with check_reconciliation(recon_job_id):
        ...
        # When Phase 3 is kicked off, leaderboard and streaks recalculations are done. To prevent
        # deadlock on the database, if a reconciliation is going on it will just set the points to 0
        # Otherwise it would assume world record incorrectly, fuck up the formula, give like a
        # bajillion points to a SmallIntField, crash the bot, and I cry.
        ...
        # With reconcilaition, we do a lot of database edits and would easily lead to a deadlock.
        # This helps prevent standing updates from occurring until after reconciliation is done.
