...
from srl.srcom.reconciliation import current_job
...
# This maps SRC's time keys to the internal short codes used through the codebase.
...
def src_method_to_internal(
    ...
    if src_value is None:
        return None
    return SRC_ALIAS.get(src_value, src_value)
...
def apply_player_obsolescence(
    ...
    Arguments:
        game_id (str): Unique SRC ID of the game.
        category_id (str | None): Unique SRC ID of the category that scopes this variant.
        variable_value_map (dict[str, str]): {variable_id: value_id} for the variant.
        player_ids (list[str]): Player IDs whose runs should be deduplicated.
        run_type (str): "main" or "il".
        level_id (str | None): Unique SRC ID of the level for IL leaderboards.
...
def update_obsolete(
    ...
    Arguments:
        ...
        level_id (str | None): Unique SRC ID of the level for IL leaderboards.
    """
    obsolete_run_ids = apply_player_obsolescence(
        game_id=game_id,
        category_id=category_id,
        variable_value_map=variable_value_map,
        player_ids=[player.id for player in players.root if player.rel != "guest"],
        run_type=run_type,
        level_id=level_id,
    )
